...
var _styleInfo2 = require('./style-info.js');

var _styleInfo3 = _interopRequireDefault(_styleInfo2);
...
var ScopingShim = function () {
  function ScopingShim() {
    ...
    ast['rules'] = [];
    this._documentOwnerStyleInfo = _styleInfo3.default.set(this._documentOwner, new _styleInfo3.default(ast));
  ...
  _createClass(ScopingShim, [{
    ...
    value: function _generateStaticStyle(info, rules, shadowroot, placeholder, cssBuild, cssText) {
      cssText = _styleTransformer2.default.elementStyles(info, rules, null, cssBuild, cssText);
      if (cssText.length) {
    ...
    value: function _prepareHost(host) {
      ...
      var cssBuild = StyleUtil.getCssBuild(template);
      var styleInfo = new _styleInfo3.default(ast, placeholder, ownStylePropertyNames, is, typeExtension, cssBuild);
      _styleInfo3.default.set(host, styleInfo);
      return styleInfo;
    }
  }, {
    ...
    value: function styleElement(host, overrideProps) {
      if (_styleSettings.disableRuntime) {
        if (overrideProps) {
          if (!_styleInfo3.default.get(host)) {
            _styleInfo3.default.set(host, new _styleInfo3.default(null));
          }
          var _styleInfo = /** @type {!StyleInfo} */_styleInfo3.default.get(host);
          this._mixOverrideStyleProps(_styleInfo, overrideProps);
          this.styleElementNativeVariables(host, _styleInfo);
        }
        return;
      }
      var styleInfo = _styleInfo3.default.get(host) || this._prepareHost(host);
    ...
    value: function _styleOwnerForNode(node) {
      ...
      var host = root.host;
      if (host) {
        if (_styleInfo3.default.get(host) || this._prepareHost(host)) {
          return host;
        } else {
    ...
    value: function _updateProperties(host, styleInfo) {
      ...
      var ownerStyleInfo = _styleInfo3.default.get(owner);
    ...
    value: function styleSubtree(host, properties) {
      ...
      var shadowChildren = root && ( /** @type {!ParentNode} */root.children || root.childNodes);
      if (shadowChildren) {
    ...
    value: function getComputedStyleValue(element, property) {
      ...
      if (!_styleSettings.nativeCssVariables) {
        ...
        var styleInfo = _styleInfo3.default.get(element) || _styleInfo3.default.get(this._styleOwnerForNode(element));
    ...
    value: function setElementClass(element, classString) {
      ...
      if (!_styleSettings.nativeCssVariables) {
        var styleInfo = _styleInfo3.default.get(element);
    ...
    value: function _styleInfoForNode(node) {
      return _styleInfo3.default.get(node);
    }
    /**
     * @param {!Element} node
     * @param {string} scope
